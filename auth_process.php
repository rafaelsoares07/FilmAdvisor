    ...
    if($name && $lastname && $email && $password && $confirmPassword ){
        ...
        if($password === $confirmPassword){
            ...
            if(strlen($password)<10){
                $message->setMessage("Sua senha deve ter no mínimo 10 caracteres", "msg-error","back");
            }
            
            else if($userDao->findByEmail($email)===false){// false quer dizer que nao retornou nenhum usuario
                $user = new User();
